...
async function purchaseWithParams(
  ...
  // Apple can return buyProductFailure + a terms action without failureType.
  ...
  if (typeof actionUrl === 'string') {
    ...
    try {
      termsRequired = new URL(actionUrl).pathname.endsWith('/termsPage');
    } catch {
      // A malformed action is not a usable terms URL.
    }
    if (termsRequired) {
      // Terms require user action, not a pricing retry or entitlement fallback.
